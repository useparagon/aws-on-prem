...
locals {
  ...
  _microservices = {
    ...
    "api-triggerkit" = {
      "port"             = lookup(local.base_helm_values.global.env, "API_TRIGGERKIT_PORT", 1725)
      "healthcheck_path" = "/healthz"
      "public_url"       = lookup(local.base_helm_values.global.env, "API_TRIGGERKIT_PUBLIC_URL", "https://api-triggerkit.${var.domain}")
    }
    "cache-replay" = {
    ...
    "worker-triggerkit" = {
      "port"             = lookup(local.base_helm_values.global.env, "WORKER_TRIGGERKIT_PORT", 1726)
      "healthcheck_path" = "/healthz"
      "public_url"       = lookup(local.base_helm_values.global.env, "WORKER_TRIGGERKIT_PUBLIC_URL", "https://worker-triggerkit.${var.domain}")
    }
    "worker-triggers" = {
      ...
      env = merge(local.base_helm_values.global.env, {
        for key, value in merge({
          ...
          local.base_helm_values.global.env,
          {
            ...
            TRIGGERKIT_POSTGRES_HOST     = try(local.base_helm_values.global.env["TRIGGERKIT_POSTGRES_HOST"], local.base_helm_values.global.env["POSTGRES_HOST"])
            TRIGGERKIT_POSTGRES_PORT     = try(local.base_helm_values.global.env["TRIGGERKIT_POSTGRES_PORT"], local.base_helm_values.global.env["POSTGRES_PORT"])
            TRIGGERKIT_POSTGRES_USERNAME = try(local.base_helm_values.global.env["TRIGGERKIT_POSTGRES_USERNAME"], local.base_helm_values.global.env["POSTGRES_USER"])
            TRIGGERKIT_POSTGRES_PASSWORD = try(local.base_helm_values.global.env["TRIGGERKIT_POSTGRES_PASSWORD"], local.base_helm_values.global.env["POSTGRES_PASSWORD"])
            TRIGGERKIT_POSTGRES_DATABASE = try(local.base_helm_values.global.env["TRIGGERKIT_POSTGRES_DATABASE"], local.base_helm_values.global.env["POSTGRES_DATABASE"])
